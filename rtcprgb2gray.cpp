...
cv::Mat RTCPrgb2gray::BGR2Gray(const cv::Mat &img)
{
    CV_Assert(img.channels()==3);
    float s = 64.f / sqrt((float)(img.rows * img.cols));
    int cols = cvRound(s * img.cols);
    int rows = cvRound(s * img.rows);
    ...
    for (int i = 0; i < cols; ++i)
    {
        ...
        {
            int x = (int)((i + 0.5f) * img.cols / cols);
            int y = (int)((j + 0.5f) * img.rows / rows);
    ...
    {
        cv::Vec3b c0 = img.at<cv::Vec3b>(pos0[i].y, pos0[i].x);
        cv::Vec3b c1 = img.at<cv::Vec3b>(pos1[i].y, pos1[i].x);
    ...
    for (int i = 0; i < cols - 1; ++i)
    {
        for (int j = 0; j < rows; ++j)
        {
            int x0 = (int)((i + 0.5f) * img.cols / cols);
            int x1 = (int)((i + 1.5f) * img.cols / cols);
            int y = (int)((j + 0.5f) * img.rows / rows);

            cv::Vec3b c0 = img.at<cv::Vec3b>(y, x0);
            cv::Vec3b c1 = img.at<cv::Vec3b>(y, x1);
    ...
    for (int i = 0; i < cols; ++i)
    {
        ...
        {
            int x = (int)((i + 0.5f) * img.cols / cols);
            int y0 = (int)((j + 0.5f) * img.rows / rows);
            int y1 = (int)((j + 1.5f) * img.rows / rows);

            cv::Vec3b c0 = img.at<cv::Vec3b>(y0, x);
            cv::Vec3b c1 = img.at<cv::Vec3b>(y1, x);
